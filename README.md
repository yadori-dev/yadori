...
> [!IMPORTANT]
> 端末から話しかけて一往復できるところまで動き、気持ちが会話で動いて時間で薄れます（一本の軸の薄い作り）。Discord と対話する道具はまだ繋がっていません。性格、気がかり、夢はこれからです。何が決まっていて何が決まっていないかは [`docs/`](docs/) を読んでください。
...
返事のあとの一行は、その往復で宿りの気持ちがどちらへどれだけ動いたか（返事を作る Claude Code が答える）と、いまの値です。値は −1（沈む）から +1（明るい）で、動きを積んで経過時間で薄めた和です（半減期 6 時間。仮置き）。今の気持ちは次の返事の前置きにも渡ります。
