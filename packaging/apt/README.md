...
[公開配布元](https://yadori-dev.github.io/yadori/)を使います。Release の発行後、配布処理が成功するとその版を取得できます。

## 利用者が導入する

次は対応 OS の端末で行います。配布元の公開鍵を置き、この配布元だけの署名確認に使います（Signed-By）。取得した鍵のフィンガープリント（鍵を識別する文字列）が、次と一致することを確認してください。

```text
4D084D7F17DA6B4483C017815840D268147C109C
```
