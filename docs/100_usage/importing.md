...
取り込みは未取り込みの完成会話を全件処理します。途中で中断しても確定済みの分は残り、再実行で残りから進みます。夢は一回分までとし、残りを表示します。外部会話は取り込んだ時点で検索でき、宿り自身の夢や気持ちへ変換しません。取り込みが0件でも自身の未処理会話があれば夢を行います。夢では既存と同様に、選んだ会話を対話する道具へ渡します。保存先を確かめられない場合は起動を断ります。
...
Claudeの既知の管理行に先頭NUL埋めがある場合は、元ログを変更せず、その行とバイト数を通知して読みます。単一ログ内で同じ発話に複数の完成応対があれば、その発話と、それを先行関係で辿れる会話の取り込み・既存照合を保留します。独立した会話は処理できます。保留の理由は件数を分けて表示します。既存の原文や先行関係は変更せず、保存済みの記録は引き続き検索できます。別ファイル間の出典の不一致や、その他の破損は従来どおり全体を停止します。

本文付きの画像発話は、既知の画像補助行と、親のRead呼出しに対応する画像だけを通って復元します。Skillの自由形式の説明も、同じ親鎖の呼出し元を確認できる場合は発話として数えません。Codexは開始・完了が重なる往復を識別子と完成本文で対応させます。同じ往復に複数の利用者発話があり対応を確定できない場合は、その往復と先行関係を辿れる往復を今回の照合・追加から保留します。保存済みの記録は変更しません。
