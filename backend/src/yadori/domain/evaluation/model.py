...
_POINTING_WORDS = re.compile(
    "|".join(
        [
            r"それ|あれ|これ|どれ|そこ|あそこ|ここ|どこ|さっき|前の|やつ|の件",
            # 直前への同意、承諾、指示だけの言い回し。話題を持たない。
            r"いいよ|いいね|いいじゃん|いいと思う|そうだね|そうしよう|そうする|そうそう|そうね",
            r"やろう|進もう|進めよう|進めて|続けて|やって|なおそう|直そう|お願い|頼む|任せる",
            r"マージして|マージしよう|マージしていい|了解|おっけ|オッケー|OK|ok|うん|はい|ありがとう",
            r"次に行こう|次に行くか|次へ|だね|かな|でしょう|ください|します|する",
        ]
    )
)
...
class Recorded:
    ...
    def has_substance(self) -> bool:
        ...
        """
        stripped = _NOT_WORDS.sub("", _POINTING_WORDS.sub("", self.utterance))
        return len(stripped) >= _SUBSTANCE_AT_LEAST
