...
不具合や改善案はIssueへ、変更はPRへ送ってください。大きな変更は実装前にIssueで範囲を相談します。コードとドキュメントへの貢献は、プロジェクトと同じ[MIT License](LICENSE)で提供してください。実際の会話や認証情報を含めず、再現には架空のデータを使ってください。
...
- **機械が名前で探すものと、利用者が端末で打つものは ASCII で名付ける。** ファイル名だけでなく、命令、下位命令、引数の名前も含む。保存先、設定ファイル、実行するものが該当する。macOS は濁点を分解して保存し、Linux はしないため、同じに見えて一致しない名前が生まれる。名前は[用語集](docs/150_system/用語集.md)のコード名に合わせる。人が読むだけの文書のファイル名は日本語でよい
...
- モックはI/O境界（外部API、DB、時刻、AIモデルの呼び出し）だけに留める。自分が書いたコードをモックしない
